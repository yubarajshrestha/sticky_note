/*
 * Project Name: Sticky Note [ using html5 web storage ]
...
Note.prototype = {
	...
	onMouseUp: function(e) {
		...
		document.removeEventListener("mouseup", this.mouseMoveHandler, true);
		this.save();
		return false;
	},
...
function loaded() {
	db.transaction(function(tx){
		tx.executeSql("SELECT COUNT(*) FROM MyStickys", [], function(result) {
			...
			tx.executeSql("CREATE TABLE MyStickys (id REAL UNIQUE, note TEXT, timestamp REAL, left TEXT, top TEXT, zindex TEXT)", [], function(result){
				loadNotes();
			});
		});
	});
